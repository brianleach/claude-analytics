...
func getAiRecommendations(
	...
	replacer := strings.NewReplacer(
		...
		"{{length_distribution}}", lenSummary,
		"{{model_usage}}", modelText,
		"{{subagent_usage}}", saText,
		"{{context_efficiency}}", ceText,
		"{{branch_activity}}", branchText,
		"{{permission_modes}}", pmText,
		"{{project_quality}}", string(pqJSON),
		"{{sample_prompts}}", sampleText,
	)
	prompt := replacer.Replace(aiPromptTemplate)
